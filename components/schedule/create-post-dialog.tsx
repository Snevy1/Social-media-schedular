    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            {/* 
                ...
                - Removed fixed h-145 from inner body; content area now uses flex-1 + min-h-0 + overflow-y-auto
                - DialogContent uses flex flex-col with a max-height so the footer is always visible
                - On mobile: full width/height with rounded-none; on sm+: normal dialog sizing
                - Right panel stacks below on mobile (flex-col) and sits beside on sm+ (flex-row)
                - Fixed heights (h-145, w-87.5, w-75, min-h-75) replaced with responsive / min-h equivalents
            */}
            <DialogContent className={cn(
                // Mobile: full screen. sm+: auto-sized dialog with max width
                "flex flex-col gap-0 px-0 pt-0 pb-0 p-0",
                "w-full h-[100dvh] max-h-[100dvh] rounded-none",
                "sm:h-auto sm:max-h-[90dvh] sm:rounded-lg",
                "sm:w-full sm:min-w-[700px]",
                selectedRightTab ? "sm:max-w-[950px]" : "sm:max-w-[700px]"
            )}>
                {/* Wrapper — flex col so header + body + footer stack, footer always at bottom */}
                <div className="flex flex-col min-h-0 flex-1 overflow-hidden">

                    {/* ── Header ── */}
                    <DialogHeader className="px-5 sm:px-8 py-3 border-b shrink-0">
                    ...
                    {/* ── Body — scrollable, fills available space ── */}
                    <div className={cn(
                        "flex flex-1 min-h-0 overflow-hidden",
                        // Stack vertically on mobile when right panel is open
                        selectedRightTab ? "flex-col sm:flex-row" : "flex-row"
                    )}>

                        {/* Left — channel selector + content */}
                        <div className={cn(
                            "flex flex-col min-w-0 overflow-y-auto",
                            // When right panel is open on mobile, cap height so both panels are accessible
                            selectedRightTab
                            ...
                            {/* Channel selector */}
                            <div className="channel--selector py-4 px-5 sm:px-8 shrink-0">
                                {channels?.length > 0 && !isPending && (
                                    <button
                                        className="mb-3 text-[13px] font-medium cursor-pointer"
                                ...
                                <div className="flex flex-wrap gap-3 sm:gap-4">
                                    {isPending ? (
                                        Array.from({ length: 6 }).map((_, index) => (
                                            <Skeleton key={index} className="size-10 sm:size-12.5 rounded-xl" />
                                        ))
                                    ) : (
                            ...
                            {/* Channel content */}
                            <div className="channel--content flex flex-col px-5 sm:px-8 pb-5 flex-1 min-h-0 overflow-y-auto">
                                        ...
                                        {selectedChannelsList?.map((channel) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {!isExpanded && (
                                                        <AccordionTrigger
                                                            className="w-full px-3 cursor-pointer [&>svg]:hidden! hover:bg-muted hover:no-underline! justify-start gap-3"
                                                        >
                                                            <span>
                                                                <HugeiconsIcon
                                                                    icon={icon}
                                                                    className="shrink-0 text-white! size-5! p-0.75 rounded-sm"
                                                                    style={{ background: channel.color }}
                                                                />
                                                            </span>
                                                            {content.text ? (
                                                                <p className="text-sm text-muted-foreground/80 truncate flex-1 text-left max-w-full">
                                                                    {content.text}
                                                                </p>
                                                            ) : (
                                                                <p className="text-sm text-muted">What would you like to share</p>
                                                            )}
                                                        </AccordionTrigger>
                                                    )}
                                                        ...
                                                        <div className="flex pt-3 px-3 gap-3">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                ...
                                                                <ContentTextarea
                                                                    ...
                                                                    onAIAssistantClick={() => setSelectedRightTab("ai")}
                                                                    onChange={(text) => handleTextChange(channel.id, text, channel.character_limit)}
                                                                    onImagesChange={(images) => {
                                                                        setChannelContent((prev) => ({
                                                                            ...prev,
                                                                            [channel.id]: { ...content, images }
                                                                        }))
                                                                    }}
                                                                    renderToolbarRight={
                        ...
                        {/* Right — ideas / AI / preview panel */}
                        {selectedRightTab && (
                            <div className={cn(
                                "flex flex-col shrink-0 border-t sm:border-t-0 sm:border-l border-border bg-muted/30",
                                // Mobile: fixed height panel; sm+: full height, fixed width
                                "h-[45dvh] sm:h-auto sm:w-87.5",
                                // Prevent the panel from shrinking on larger screens
                                "sm:shrink-0"
                            )}>
                                <div className="py-4 flex-1 flex flex-col h-full overflow-y-auto">
                                    {selectedRightTab === "ai" && (
                                        <div className="px-4 sm:px-6">
                                            <AIAssistant
                                                content={channelContent[activeAccordion]?.text || globalContent?.text || ""}
