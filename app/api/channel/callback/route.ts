...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        // Twitter and TikTok both use PKCE
        const codeVerifier = (
            state.channelType === ChannelTypeEnum.TWITTER ||
            state.channelType === ChannelTypeEnum.TIKTOK
        ) ? request.cookies.get(pkceCookieName)?.value : undefined
        ...
        console.log("callback profile", JSON.stringify(profile, null, 2))

        // Instagram: store the page access token instead of the user token.
        // The page token is what the Graph API requires for content publishing.
        const storedAccessToken = (
            state.channelType === ChannelTypeEnum.INSTAGRAM && profile.pageAccessToken
        ) ? profile.pageAccessToken : token.accessToken

        // Instagram page tokens are long-lived (60 days) and don't have a
        // standard expiresAt from the token exchange, so we null it out
        // to avoid premature refresh attempts.
        const storedExpiresAt = (
            state.channelType === ChannelTypeEnum.INSTAGRAM && profile.pageAccessToken
        ) ? null : (token.expiresAt ?? null)
        ...
        const payload = {
            ...
            access_token: encrypt(storedAccessToken),
            refresh_token: encrypt(token.refreshToken ?? null),
            token_expires_at: storedExpiresAt,
            is_connected: true,
            is_active: true,
        }
        ...
        if (error) {
            console.error("Failed to upsert user channel", error)
            const response = buildRedirectUrl(APP_URL, redirectTo, {
                connected: "false",
